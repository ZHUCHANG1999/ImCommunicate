...
func (u Users) LastInsertId() (int64, error) {
	//TODO implement me
	panic("implement me")
}

func (u Users) RowsAffected() (int64, error) {
	//TODO implement me
	panic("implement me")
}
...
func (m *defaultUsersModel) ListByName(ctx context.Context, name string) ([]*Users, error){
	query := fmt.Sprintf("slect %s from % where `nickName` like ?", usersRows, m.table)

	var resp  []*Users
	err := m.QueryRowsNoCacheCtx(ctx, &resp, query, fmt.Sprintf("%", name, "%"))
	switch err {
	case nil:
		return resp, nil
	default:
...
func (m *defaultUsersModel)  ListByIds(ctx context.Context, ids []string) ([]*Users, error){
	...
	query := fmt.Sprintf("slect %s from %s where `id` in ('%s')", usersRows, m.table,strings.Join(ids,"',"))
